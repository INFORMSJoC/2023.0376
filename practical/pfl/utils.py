...
from pfl.optim.utils import MD
...
def make_pfl_train_parser():
    ...
    pfl_parser.add_argument('--pfl_algo', type=str, required=True,
        choices=['fedpop', 'fedavg', 'fedalt', 'fedsim', 'pfedme']
    )
    ...
    # fedpop argument
    pfl_parser.add_argument('--laplace_coef', type=float, default=1e-5)
    pfl_parser.add_argument('--n_canonical', type=int, default=2)  # the number of potential canonical model

    # Federated Training Arugments
    fed_parser = parser.add_argument_group('train_args', 'Model training args')
    fed_parser.add_argument('--client_optimizer', type=str, default='sgd', choices=['sgd', 'adam', 'md'])
...
def make_finetune_parser():
    ...
    parser.add_argument('--optimizer', type=str, default='sgd', choices=['sgd', 'adam', 'md'])
...
def add_common_args(parser):
    ...
    parser.add_argument('--commondir', type=str, default='./checkpoint/pfl')
    parser.add_argument('--interpolate', action='store_true')  # Interpolate or Mixing predictions in Pop
    parser.add_argument('--pretrain', action='store_true')  # weather pretrain or train
    parser.add_argument('--num_neighbor', type=int, default=100)  # if sketching, the selected number.
    parser.add_argument('--save_model_every_n_round', type=int, default=500)
...
def setup_personalized_optimizer_from_args(args, model, num_training_steps):
    if args.optimizer == 'sgd':
        ...
        optimizer = MD(model.parameters(), lr=args.lr)
    else:
