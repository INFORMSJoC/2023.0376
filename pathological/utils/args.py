...
def args_to_string(args):
    ...
    if args.method == "Fedpop":
        args_string += f"_{args.n_canonical}"
        if args.interpolate:
            args_string += '_interpolate'

    args_string += f'_{args.seed}'
...
def parse_args(args_list=None):
    ...
    parser.add_argument(
        ...
        help='the method to be used;'
             ...
             ' `pFedMe`, `AFL`, `FFL`, `clustered`, `FedLG` and `Fedpop`;',
        type=str
    )
    parser.add_argument(
        '--decentralized',
    ...
    parser.add_argument(
        '--input_dimension',
        help='the dimension of one input sample; used for synthetic and har dataset',
        type=int,
        default=None
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--include_keyword",
        help='only used in FedLG, specific classifier in cifar10',
        type=str,
        default=None
    )
    parser.add_argument(
        "--exclude_keyword",
        help='only used in Fedpop',
        type=str,
        default=None
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--n_canonical",
        help='the number of canonical models, used only in Fedpop',
        type=int,
        default=1
    )
    parser.add_argument(
        "--client_lr",
        help="the learning rate of canonical parameters. used only in Fedpop",
        type=float,
        default=0.5,
    )
    parser.add_argument(
        "--client_scheduler",
        help="the client optimizer scheduler. used only in Fedpop",
        type=str,
        default="multi_step"
    )
    parser.add_argument(
        "--laplace_coef",
        help="the laplace penalty coefficient in Fedpop",
        type=float,
        default=1e-5
    )
