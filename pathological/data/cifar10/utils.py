...
def split_dataset_by_labels(dataset, n_classes, n_clients, n_clusters, alpha, frac, seed=1234):
    ...
    # record sample contained in clusters
    ...
    for _, cluster in clusters.items():
        rng.shuffle(cluster)  # shuffle client order

    clients_counts = np.zeros((n_clusters, n_clients), dtype=np.int64)  # number of samples by client from each cluster

    for cluster_id in range(n_clusters):
        weights = np.random.dirichlet(alpha=alpha * np.ones(n_clients))  # a distribution denotes the cluster samples proportion of in each client
        clients_counts[cluster_id] = np.random.multinomial(clusters_sizes[cluster_id], weights)

    clients_counts = np.cumsum(clients_counts, axis=1)

    clients_indices = [[] for _ in range(n_clients)]  # record samples indices in each clients.
    for cluster_id in range(n_clusters):
