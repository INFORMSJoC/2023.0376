...
class LearnersEnsemble(object):
    ...
    def __init__(self, learners, learners_weights):
        ...
        self.include_keyword = self.learners[0].include_keyword
        self.exclude_keyword = self.learners[0].exclude_keyword
        self.used_mirror = self.learners[0].used_mirror  # new
    ...
    def mirror_step(self, iterator, n_epochs, optimizer, scheduler, penalty=None):
        assert len(self.learners) ==  1
        # old_factor = self.learners[0].canonical_parameters()
        self.learners[0].mirror_step(iterator, n_epochs, optimizer, scheduler, penalty)
        # new_facotor = self.learners[0].canonical_parameters()
