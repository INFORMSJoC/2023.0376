...
from utils.optim import get_lr_scheduler, MD
...
class Client(object):
    ...
    def step(self, single_batch_flag=False, *args, **kwargs):
        ...
        """
        self.counter += 1
        self.update_sample_weights()  # only for EM
        self.update_learners_weights()  # only for EM
...
class FedpopClient(Client):

    def __init__(self, learners_ensemble, train_iterator, val_iterator, test_iterator, logger, local_steps, client_lr, client_scheduler, tune_locally=False):
        super().__init__(learners_ensemble, train_iterator, val_iterator, test_iterator, logger, local_steps, tune_locally)
        
        self.canonical_factor = self.learners_ensemble[0].model.canonical_factor
        self.n_canonical =self.canonical_factor.shape[0]

        self.canonical_optimizer = MD([self.canonical_factor], client_lr)
        self.canonical_scheduler = get_lr_scheduler(self.canonical_optimizer, client_scheduler, n_rounds=200)
        self.canonical_step = 0
    
    def mirror_step(self, penalty=None):
        self.canonical_step += 1

        self.learners_ensemble.mirror_step(
            iterator = self.train_iterator,
            n_epochs = self.local_steps,
            optimizer = self.canonical_optimizer,
            scheduler = self.canonical_scheduler,
            penalty = penalty
            )
